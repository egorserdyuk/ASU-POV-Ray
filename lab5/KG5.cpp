	...
	int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_KG5, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_KG5));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_KG5));
	wcex.hCursor = LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wcex.lpszMenuName = MAKEINTRESOURCE(IDC_KG5);
...
bool PavlidPolyClip(int& x1, int& y1, int& x2, int& y2, std::vector<std::vector<int>> vec) // координаты начала и конца отрезка и массив с координатами многоугольника
{
	...
	if (count0 == 1 && count1 == 0) // 2, прямая касается многоугольника, то есть всего одна общая точка
		return false;

	// случаи 3, 4 и 5 из презентации объеденены в общий алгоритм
	for (i = 0; i < u.size() - 1; i++)
	{
		...
		{
			// в coord засовываем точку пересечения исходной прямой и прямой, содержащей это ребро
			int x3, y3, x4, y4;
			x3 = vec[i][0];
			y3 = vec[i][1];
			x4 = vec[i + 1][0];
